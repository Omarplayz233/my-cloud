<script lang="ts">
  import {
    IconFolder, IconFile, IconPhoto, IconVideo, IconMusic,
    IconFileZip, IconFileText, IconDownload, IconEye,
    IconChevronRight, IconCloud,
  ...
  type FileRecord = {
    ...
    time: string; metaFileId: string; folderId?: string; public?: boolean;
  };
  type FolderRecord = {
    _type: 'folder'; folderId: string; name: string;
    createdAt: string; parentId?: string; public?: boolean;
  };
...
<div class="root">
  <header>
    <div class="header-left">
      <IconCloud size={20} /><span class="header-title">Omar's Cloud</span>
    </div>
  ...
  <main class="main">
    ...
    <div class="folder-header">
      ...
      <div>
        ...
        <p class="folder-meta">{files.length} file{files.length !== 1 ? 's' : ''}{subfolders.length > 0 ? ` · ${subfolders.length} folder${subfolders.length !== 1 ? 's' : ''}` : ''}</p>
      </div>
    </div>
    ...
    {#if subfolders.length > 0}
      ...
      <div class="subfolder-grid">
        {#each subfolders as sub (sub.folderId)}
          <a class="subfolder-card" href="/public/folder/{folderPath}/{sub.name}">
...
<style>
  ...
  :global(body) { background: var(--bg-1); color: var(--text-1); font-family: 'Geist', system-ui, sans-serif; min-height: 100vh; }
  ...
  .header-left {
    display: flex; align-items: center; gap: 9px; color: var(--text-1);
  }
  .header-title {
  ...
  .header-badge {
    ...
    color: var(--text-3); border: 1px solid var(--border); border-radius: 6px; padding: 3px 8px;
  }

  /* Main */
  .main { max-width: 860px; margin: 0 auto; padding: 32px 24px 80px; width: 100%; flex: 1; }

  /* Breadcrumb */
  .breadcrumb { display: flex; align-items: center; gap: 6px; margin-bottom: 28px; flex-wrap: wrap; }
  .bc-item { display: flex; align-items: center; gap: 4px; font-size: 12.5px; color: var(--text-3); text-decoration: none; padding: 3px 6px; border-radius: 6px; transition: .14s; }
  ...
  .section-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .08em; color: var(--text-3); margin-bottom: 8px; margin-top: 4px; }

  /* Subfolders */
  .subfolder-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px; margin-bottom: 28px; }
  .subfolder-card {
  ...
  .sub-name { flex: 1; font-size: 13px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  :global(.sub-arrow) { color: var(--text-3); flex-shrink: 0; }

  /* File list */
  .file-list { display: flex; flex-direction: column; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
  .file-row { display: grid; grid-template-columns: 32px 1fr 80px 100px 80px; align-items: center; padding: 0 12px; min-height: 44px; border-bottom: 1px solid var(--border); transition: background .12s; }
  .file-row:last-child { border-bottom: none; }
  .file-row:not(.file-row-header):hover { background: var(--hover); }
  .file-row-header { background: var(--bg-2); min-height: 36px; }
  .fh-icon,.fh-name,.fh-size,.fh-date,.fh-actions { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .06em; color: var(--text-3); }
  .ficon { display: flex; align-items: center; color: var(--text-3); }
  .fname { overflow: hidden; }
  .fname-btn { background: none; border: none; color: var(--text-1); font-size: 13px; font-family: inherit; cursor: pointer; padding: 0; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 100%; text-decoration: none; display: block; }
  ...
  /* Act btn */
  .act-btn { display: flex; align-items: center; gap: 5px; background: none; border: none; color: var(--text-3); cursor: pointer; padding: 5px; border-radius: 6px; transition: .12s; font-size: 12px; font-family: inherit; text-decoration: none; }
  ...
  .preview-close {
    ...
    background: rgba(0,0,0,.6); border: none; color: var(--text-2);
  ...
  .preview-name { font-size: 13px; color: var(--text-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 60%; }
